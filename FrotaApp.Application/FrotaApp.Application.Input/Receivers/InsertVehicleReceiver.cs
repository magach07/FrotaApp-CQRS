...
namespace FrotaApp.Application.Input.Receivers
{
    public class InsertVehicleReceiver : IReceiver<VehicleCommand, State>
    {
        ...
        {
            var vehicle = new VehicleEntity(command.Name, command.Color, command.ModelYear, 
            command.CategoryId, command.Price, command.Type);

            if(!vehicle.IsValid())
                return new State(400, "Falha ao inserir, favor verifique os campos.", vehicle.Notifications);

            try
            {
                _repository.InsertVehicle(vehicle);
                return new State(200, "Veiculo adicionado com sucesso", vehicle);
            }
            catch (Exception ex)
            {
